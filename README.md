...
- 检测页优先显示异常，已通过项与缓存详情默认折叠；所有原有检查继续执行。
- 诊断摘要与日志分区，日志类型选择和读取按钮保持同一行。
- 统一手机窄屏、深浅色和系统栏安全边距，保留默认非全屏与中英文设置。
- 直接打开监控或设置时不运行完整诊断，进入数据页面或手动刷新后才读取。监控结果未变时复用进程列表，过期时仍清空显示。
- 清理重复 CSS，无新增框架、字体、动画或后台常驻任务。动态网络 APEX 适配和 0.7.2 启动修复保留。

[下载 0.8.0 发布版](https://github.com/MiToverG422/ColorOS-Global-TangoBridge/releases/tag/v0.8.0)。适配范围及实验性运行库限制不变。
...
0.7.1 增加适配可观测性：检测页展示最近阶段、失败原因、缓存份数及运行缓存标识；完整检测分别验证缓存完整性、当前输入与运行缓存一致性、以及本次启动的 JNI 验证记录。诊断页可选择启动、生成、JNI 三类日志，每次最多读取末尾 64 KiB 中的 120 行。启动日志超过 128 KiB 时，在下次启动保留末尾 64 KiB 到上一份日志，避免长期累积。

现有后台监控开关同时控制适配阶段采样，无新增常驻进程。每轮只读取一份原子更新的小型状态记录；重启前记录和不完整记录不会作为当前状态。这里监视的是最近适配操作，不轮询网络 JAR 变化、不自动重启服务，也不重复运行 JNI 探针；完整哈希检查仅在启动及手动检测时进行。阶段耗时是记录产生时的耗时，不是持续运行的计时器。JNI 通过仍仅代表启动探针的结果。
...
修复动态网络 JAR 的 bind 源路径仍可访问时，Zygote 在派生应用进程阶段报 `Not allowlisted` 并退出的问题。仅在设备号和 inode 一致时返回对应标准 APEX 路径，保留原有允许列表。此次手机测试中网易云音乐 8.6.76 和第五人格离线研究版 1.5.4-offline.76 均以 ARM32 启动，用户确认可进入。

诊断与后台监控增加已验证 Zygote 的 PID 和启动时刻比对，服务退出或重启后不复用旧验证结果。概览文字改为“运行环境就绪”，明确环境检查不代表所有 App 兼容。
